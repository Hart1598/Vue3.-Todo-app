  ...
  <main>
    ...
    <ul class="my-list task-list">
      <li v-for="item in taskList" :key="item.id">
        <TaskCard @onRemove="removeTask(item.id)" @onDone="setDoneTask(item.id)" :model="item"/>
      </li>
    </ul>
  </main>
</template>
...
export default {
  ...
  setup(){
    const taskList = ref([{id:0, title:'Create Video', description: 'And upload', status:false}])
